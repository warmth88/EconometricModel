...
clear 
%addpath('D:\Dropbox\MATLAB\gensys')
addpath('C:\Users\eldragon\Dropbox\gensys')
%%
%GLOBALS
...
%CALL SOLUTION
disp('COMPUTING!!!');
[Fss,FLHss,FRHss] = dd_fsys_jm_7_2016(Xss,Xss,etass,epsss); 
...
for Xct=1:numstates;
   ...
   X(Xct) = X(Xct)+h;
   F = dd_fsys_jm_7_2016(X,Xss,etass,epsss);
   F1(:,Xct) =(F-Fss)/h;
end
...
for Xmin1ct=1:numstates;
   ...
   F = dd_fsys_jm_7_2016(Xss,Xmin1,etass,epsss);
   F2(:,Xmin1ct) = (F-Fss)/h;
end
...
for etact=1:numetas;
   ...
   F = dd_fsys_jm_7_2016(Xss,Xss,eta,epsss);
   F3(:,etact) = (F-Fss)/h;
end
...
for epsct = 1:numeps;
    ...
    F = dd_fsys_jm_7_2016(Xss,Xss,etass,eps);
    F4(:,epsct) = (F-Fss)/h;
end
...
resp1b(:,1)=squeeze(resp(Brkind,1,:)); % "squeeze" eliminates the singleton dimensions
% of resp(:,:,:). It returns a matrix with the first ten
% responses of the 1st variable to the 1st shock
resp2b(:,1)=squeeze(resp(Brkind,2,:));
resp3b(:,1)=squeeze(resp(Brkind,3,:));
figure(2)
subplot(3,1,1)
plot(1:10,resp1b(:,1))
