function [F, FLHS, FRHS, NDPOL, BPOL] = dd_fsys(X, Xmin1, eta, eps) 
...
FRHS(Brkind) = Br;
FRHS(Bind)  = - (1 - DeltaB - Delta)/(1 + Pimin1) & Brkmin1 - (1 + Imin1) / (1 + Pimin1) * Bgmin1 + Wmin1val * ZKmin1 * NKmin1 + Dkmin1 + Dgmin1 - Tkmin1 - Ckmin1;%CONSUMPTION IS THE PROBLEM
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FRHS(Bgind)  = -Gmin1 + (1 + Imin1)/(1 + Pimin1) * Bgmin1 + Tkmin1; %Fundamentals aren't set to balance
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

FRHS(Wind)   = Mmin1 * (1 - alpha) * Amin1 * Kmin1^alpha * Lmin1^(- alpha);%CHECK
FRHS(Tkind)  = Tkbar + gamma_t * log( ( Bgmin1 / Ymin1) / bybar); %CHECK
