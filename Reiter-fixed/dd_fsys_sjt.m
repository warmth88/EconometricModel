...
Rmin1       = Xmin1(Rind);
Ikmin1      = Xmin1(Ikind);
Mlagmin1 = Xmin1(Mlagind);
Bglagmin1 = Xmin1(Bglagind);
...
FRHS(Bgind)  = -Gmin1 + ((1 + Imin1)/(1 + Pimin1)) * Bglagmin1 + Tkmin1;
FRHS(Wind)   = Mmin1 * (1 - alpha) * Amin1 * Kmin1^alpha * Lmin1^(- alpha);
% FRHS(Wind)   = (1 - alpha) * Amin1 * Kmin1^alpha * Lmin1^(- alpha);
