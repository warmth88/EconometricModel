...
%FRHS(Kind)   = Lambda_t_t1 * ( R + 1 + xik * ( IK / K - deltak) * (IK / K - deltak + 1) - xik/2 * (IK / K - deltak)^2) + eta(etaKind); 
FRHS(Kind)   = ( (Lambda_t_t1 * ( R + 1 + xik * ( IK / K - deltak) * (IK / K - deltak + 1) - xik/2 * (IK / K - deltak)^2) - 1)/xik + 1 )*Kmin1-1 + eta(etaKind) ; 

FRHS(Gind)   = -log(Ymin1) + log(gybar) - gamma_g * log(Bgmin1 / Ymin1); %NEED  TO SET NON-ZERO LEVEL OF GOVERNMENT SPENDING
