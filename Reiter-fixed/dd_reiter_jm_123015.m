...
%CALL SOLUTION
disp('COMPUTING!!!');
[Fss,FLHss,FRHss] = dd_fsys_jm_123015(Xss,Xss,etass,epsss); 
...
for Xct=1:numstates;
   ...
   X(Xct) = X(Xct)+h;
   F = dd_fsys_jm_123015(X,Xss,etass,epsss);
   F1(:,Xct) =(F-Fss)/h;
end
...
for Xmin1ct=1:numstates;
   ...
   F = dd_fsys_jm_123015(Xss,Xmin1,etass,epsss);
   F2(:,Xmin1ct) = (F-Fss)/h;
end
...
for etact=1:numetas;
   ...
   F = dd_fsys_jm_123015(Xss,Xss,eta,epsss);
   F3(:,etact) = (F-Fss)/h;
end
...
for epsct = 1:numeps;
    ...
    F = dd_fsys_jm_123015(Xss,Xss,etass,eps);
    F4(:,epsct) = (F-Fss)/h;
end
...
%test determinacy of just aggregate part
[G1t,Ct,impactt,fmatt,fwtt,ywtt,gevt,eut]= ...
    gensys(g0(Ckind:end,Ckind:end),g1(Ckind:end,Ckind:end),c(Ckind:end,1),psi(Ckind:end,1),pi(Ckind:end,:));
%Model is now solved
