...
%CALL SOLUTION
disp('COMPUTING!!!');
[Fss,FLHss,FRHss, NDPOL, BDPOL] = dd_fsys_jm_2(Xss,Xss,etass,epsss); 

ndpolsstemp    = importdata('ndensepol.txt');
ndpolss = reshape(ndpolsstemp,bdensenum,znum);	
diffnd = ndpolss - NDPOL;

bpolsstemp    = importdata('bdensepol.txt');
bdpolss = reshape(bpolsstemp,bdensenum,znum);	
diffnd = bdpolss - BDPOL;
%plot(1:bnum,FLHss(VNDind:VNDind + bnum-1),1:bnum,FRHss(VNDind:VNDind + bnum-1))
%for iz = 1: znum
%    for ib = 1:bdensenum
%        ndpolss(iz,ib) = ndpolsstemp(
